...
当前实现只公开 `ReplayRequestPayload`。旧研究名称由 `replay_presets.py` 在客户端生成同一 payload；核心 manifest 保存 `replay_protocol`、extractor observation、binding observation 和可选 `comparison_results`，不保存 pair protocol 或 mode verdict。

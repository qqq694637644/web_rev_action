...
Typical errors: `invalid_operation_payload`, `experiment_not_found`, `capture_uuid_mismatch`, `invalid_adapter_response`. Missing or invalid live `capture`, `requests`, or `pagination` fields are protocol failures, not empty stream state. The returned numeric capture ID must match the requested private handle, and pagination page/index/next-page facts must be mutually consistent.

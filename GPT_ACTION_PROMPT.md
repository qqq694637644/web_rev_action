...
`browser/dispatcher.py`；capture lifecycle、replay preparation/dispatch、replay analysis、
evidence collection/observation assembly、finalization、inspection 和 session 分属专用 operation
模块，模块使用显式 import，不共享 wildcard namespace。Replay JavaScript 位于
`browser/replay_runtime.js`。新增 transport 实现分别进入
`browser/adapters/command.py|playwright.py|mcp.py|js_reverse.py`，新增 analyzer 放入
...
Replay 的 exact `replay_request` network selector 由服务内部保留并始终注入。自定义
`network_evidence` 只用于 supporting evidence，不得使用保留 ID。Remove/replace/extractor
的整数 occurrence 必须非负；add header/query 只使用 `append`。

有序操作审计要区分“已应用到 spec”和“最终 wire 仍可独立观察”。被后续 operation 覆盖
时读取 `final_wire_observability=overwritten_by_later_operation`，不要解释为执行失败。
...
终止审计必须同时检查 `observed_termination`、`terminal_condition_matched`、
`termination_reason_matches_conditions` 和
`terminal_condition_matches_observed_termination`。空 conditions 会规范化为 network close。

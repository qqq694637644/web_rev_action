...
判断结果时查看 `execution.status` 和可重算的 `quality_summary`。每个请求的事实、association confidence、completeness 和 missing evidence 只从 `network_observations[]` 读取；artifact 完整性从 `artifacts[].completeness` 读取。不要再查找已删除的 execution/evidence/collector/primary integrity 顶层字段。普通 network source 不能把缺失的 raw.bin、event JSONL 或 stream metadata升级为 complete。

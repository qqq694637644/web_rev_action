...
inspectBrowserEvidence 只读，用于查询 session、experiment、stream 状态，读取、搜索、分页和导出本地 evidence。runBrowserExperiment 用于 open_session、capture_baseline、capture_flow 和 close_session，会执行页面操作并触发确认。
...
primary_request 必须明确 url、method、resource_types 和 mime_types。事件正文谓词由 collector 内部匹配；不要尝试读取整个 events.jsonl 后自行扫描。需要搬运 evidence 时使用 runBrowserExperiment(export_experiment) 创建 ZIP，不要假设能访问 Action 服务本机目录或 GitHub Gateway workspace。

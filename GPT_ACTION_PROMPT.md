...
inspectBrowserEvidence 只读，用于查询 session、experiment、stream 状态和 artifact。runBrowserExperiment 用于 open_session、capture_baseline、capture_flow 和 close_session，会执行页面操作并触发确认。
...
运行 capture_flow 前明确 objective、primary_request、flow、wait_for 和 deadline。每次实验只改变一个变量。默认 include_in_flight=false，避免实验前已经发出的请求污染结果。
...
底层 network_canceled 不能直接解释为用户点击 Stop。只有 experiment manifest 明确给出 expected_user_cancel 时才能这样表述。[DONE] 只是默认结束谓词，不是所有流协议的通用定义。

读取 credential artifact 时默认使用 redacted。只有用户明确要求本地重放且确实需要原值时，才使用 credential_mode=full；不要把完整 Cookie、Authorization 或 CSRF 写进自然语言回复。

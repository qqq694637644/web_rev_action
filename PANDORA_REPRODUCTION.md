...
字段必要性实验使用成对 `replay_request`。先执行无 mutation 的 control，并用 volatile bindings 生成新的 message/conversation/idempotency/timestamp 值；再让 treatment 引用 control、复用同一组值并只修改一个字段。后端本地读取 exact request artifact，在当前登录页面上下文执行 fetch，并用 actual outbound snapshot 验证 mutation 是否生效。凭据不进入 Skill、Action 响应或报告。
...
Browser-managed Cookie、Origin、Referer、Host、Content-Length 和 `Sec-*` header mutation 会被拒绝。Source 为 `text/event-stream` 时，replay 自动启用 stream collector；每个 primary stream 生成 `stream_request` 和 `stream_event_range` evidence。若有效 treatment 返回明确非流 4xx，记录为协议拒绝而不是 collector 故障。
...
完整 credential artifact 默认被 workspace inspect/search/read 隐藏。后端 replay 可以按 evidence ID 本地使用；本机专家只有显式 `include_credentials=true` 才能读取正文。

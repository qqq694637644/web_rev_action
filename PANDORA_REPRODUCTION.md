...
```text
fresh_equivalent  默认用于 message/request ID、nonce、timestamp
same_value        仅显式用于 conversation ID、固定 parent/context
```

Treatment 只传 `control_experiment_id + mutation`，其余参数从 Control 的 immutable `pair_protocol_hash` 继承。Fresh 值可不同，后端会规范化后比较非目标字段。只有 Control target baseline存在、Treatment target delta正确、volatile bindings实际出现在 wire、非目标字段等价且环境指纹等价时，mutation才有效。凭据不进入 Skill、Action 响应或报告。
...
普通 network evidence 在第一条 mutation 前记录 reqid high-water mark。Finalization 只接受窗口内请求；对 selector 命中的 reqid在 MCP generation 仍有效时导出 exact headers/body/initiator，并生成稳定 evidence ID。JSON request 同时生成 `request-shape.json` 和 `request-body.redacted.json`，使用无 wildcard JSON Pointer显示数组和对象路径，例如 `/messages/0/content/parts/0`。`get_request_shape` 默认分页返回路径；redacted body只有显式请求时才按 depth/array limit返回裁剪子树。

Browser-managed Cookie、Origin、Referer、Host、Content-Length 和 `Sec-*` header mutation 会被拒绝。Source 为 `text/event-stream` 时，replay 自动启用 stream collector；evaluate通过增量 reader在 done marker、byte limit、idle timeout或network close终止。Raw 与 EventSource mirror分别生成自己的 `stream_event_range`，不混用 count。

有效 Treatment 返回非流错误响应时，raw/semantic维度标记为 `not_applicable_non_stream_response`，而不是 collector失败。错误分类：

```text
validation_rejection      400/409/422 且明确引用 mutation target
...
只有 `validation_rejection` 可支持 required；其他结果必须标记 partial/inconclusive。

...
字段必要性实验使用 `replay_request`。Skill 只传 source experiment/evidence 和结构化 mutation；后端本地读取 exact request artifact，在当前登录页面上下文执行 fetch。凭据不进入 Skill、Action 响应或报告。
...
普通 network evidence 在第一条 mutation 前记录 reqid high-water mark。Finalization 只接受窗口内请求；对 selector 命中的 reqid在 MCP generation 仍有效时导出 exact headers/body/initiator，并生成稳定 evidence ID。
...
inspectBrowserEvidence.get_request_initiator
  ↓
search_scripts → get_script_source
  ↓
replay_request: 只删除一个低风险 tracking/optional 字段
  ↓
比较 replay response、network evidence、page state 和持久状态
  ↓
